...
# Monitor Twitter feeds to better understand customer sentiment using Watson Assistant for Business, Tone Analyzer, and Natural Language Understanding

In this journey, our server application subscribes to a Twitter feed as configured by the user. Each tweet received will be analyzed for emotional tone and sentiment, and the intent of the tweet will be determined by the Watson Assistant for Business service. All data is stored in a Cloudant database, with the opportunity to store historical data as well. The resulting analysis is presented in a Web UI as a series of graphs and charts.
...
* Send the tweets to Watson Tone Analyzer, Assistant for Business, and Natural Language Understanding for processing and analysis.
...
5. The Watson Assistant for Business Service extracts the intents (verbs) from the tweets.
...
* [Watson Assistant for Business](https://www.ibm.com/watson/services/conversation): Build, test and deploy a bot or virtual agent across mobile devices, messaging platforms, or even on a physical robot.
...
5. [Import the Assistant for Business workspace](#5-import-the-conversation-workspace)
...
We’ll be using the file [`data/conversation/workspaces/workspace-social-crm-airline-classification.json`](data/conversation/workspaces/workspace-social-crm-airline-classification.json) to configure our Watson Assistant for Business workspace.
...
6. Create the required services and bind them to the newly created application: `Watson Assistant for Business`, `Natural Lanuguage Understanding`, `Tone Analyzer`, and `Cloudant NoSQL DB`.
...
* [**Watson Assistant for Business**](https://console.bluemix.net/catalog/services/conversation)
...
### 5. Import the Assistant for Business workspace
...
The credentials for IBM Cloud services (Assistant for Business, Tone Analyzer,
Natural Language Understanding, and Cloudant), can be found in the ``Services`` menu in IBM Cloud,
by selecting the ``Service Credentials`` option for each service.

The other setting for Assistant for Business was collected during the
...
When you select the `Twitter` option on the main menu of the setup utility, you will be required to enter the Twitter Tokens first.  Continue to enter the screen name you are listening to, the Assistant for Business API workspace id for the classifications, and finally the Chatbot screen name and Assistant for Business API workspace for the Dialog implementation.
